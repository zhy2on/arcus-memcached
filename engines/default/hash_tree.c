 ...
 * Returns the space added for the new node (to be reflected by caller). */
                       ...
                       htree_hash_node *node)
{
    ...
    return slabs_space_size(sizeof(htree_hash_node));
}
 ...
 * Returns the space freed for the removed node (to be reflected by caller). */
                                    ...
                                    const void *cookie)
{
    ...
    if (find != NULL) {
        ...
        if (space_delta_out) {
            ssize_t old_space = (ssize_t)slabs_space_size(htree_elem_ntotal(find));
            ssize_t new_space = (ssize_t)slabs_space_size(htree_elem_ntotal(elem));
            *space_delta_out = new_space - old_space;
        }
        if (replaced_out)
            *replaced_out = find;
    ...
    if (node->hcnt[hidx] >= HTREE_MAX_HASHCHAIN_SIZE) {
        ...
        size_t node_space = htree_node_link(root, node, hidx, n_node);
        if (space_delta_out) *space_delta_out += (ssize_t)node_space;
        node = n_node;
        hidx = HTREE_GET_HASHIDX(elem->hval, node->hdepth);
    }
    ...
    if (space_delta_out)
        *space_delta_out += (ssize_t)slabs_space_size(htree_elem_ntotal(elem));
    return ENGINE_SUCCESS;
}
    ...
    for (hidx = 0; hidx < HTREE_HASHTAB_SIZE; hidx++) {
        if (node->hcnt[hidx] == -1) {
            ...
            htree_elem_item *found = htree_elem_at_offset(root, child_node, offset,
                                                          delete, on_elem_delete,
                                                          space_delta_out);
            if (delete) {
                if (child_node->tot_elem_cnt < (HTREE_MAX_HASHCHAIN_SIZE/2)
                    && htree_node_is_leaf(child_node)) {
                    size_t freed = htree_node_unlink(root, node, hidx);
                    if (space_delta_out) *space_delta_out -= (ssize_t)freed;
                }
                node->tot_elem_cnt -= 1;
            }
            return found;
            ...
            if (delete) {
                ...
                if (space_delta_out)
                    *space_delta_out -= (ssize_t)slabs_space_size(htree_elem_ntotal(elem));
                if (on_elem_delete) on_elem_delete(elem, ELEM_DELETE_NORMAL);
    ...
    for (hidx = 0; hidx < HTREE_HASHTAB_SIZE; hidx++) {
        if (node->hcnt[hidx] == -1) {
            ...
            int child_fcnt = htree_traverse_dfs_bycnt(root, child_node, rcnt, delete,
                                                      ...
                                                      on_elem_delete, cause, space_delta_out);
            fcnt += child_fcnt;
            ...
            while (elem != NULL) {
                ...
                if (delete) {
                    ...
                    if (space_delta_out)
                        *space_delta_out -= (ssize_t)slabs_space_size(htree_elem_ntotal(elem));
                    if (on_elem_delete) on_elem_delete(elem, cause);
                    if (elem->refcount == 0) htree_elem_free(elem);
                }
                                          ...
                                          ssize_t *space_delta_out)
{
    ...
    if (node->hcnt[hidx] == -1) {
        ...
        ret = do_htree_traverse_dfs_byfield(root, child_node, hval, key, klen,
                                            delete, elem_array, on_elem_delete,
                                            space_delta_out);
        if (ret && delete) {
            if (child_node->tot_elem_cnt < (HTREE_MAX_HASHCHAIN_SIZE/2) &&
                htree_node_is_leaf(child_node)) {
